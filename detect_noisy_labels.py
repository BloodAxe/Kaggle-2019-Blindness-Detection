...
def main():
    ...
    valid_negatives = glob.glob('C:/Develop/Kaggle/Kaggle-2019-Blindness-Detection/negative-mining/negatives_fold0/valid/*.csv') + \
                      glob.glob('C:/Develop/Kaggle/Kaggle-2019-Blindness-Detection/negative-mining/negatives_fold1/valid/*.csv')

    print(len(valid_negatives))
    mistakes_per_sample, all_mistakes = count_mistakes(valid_negatives)

    df1 = all_mistakes.groupby(['image_id', 'y_true', 'y_pred']).size().sort_values(ascending=False).reset_index(name='count')
